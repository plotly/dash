...
export default function DateRangeSlider({
    ...
    const mappedStep = useMemo(() => {
        if (delta_years || delta_months || delta_days) {
            const ref = parsedMin ?? new Date();
            const stepped = stepDate(
                ref,
            ...
            return stepped.getTime() - ref.getTime();
        }
        if (marks) {
            return null;
        }
        return MS_PER_DAY;
    }, [parsedMin, delta_years, delta_months, delta_days, marks]);

    // String representation of step for use in snapping logic
    const step =
        delta_years || delta_months || delta_days
            ? [delta_years, delta_months, delta_days]
                  .map(n => Number(n))
    ...
    useMemo(() => {
        ...
        win.dccFunctions[formatFuncName] = (timestamp: number) => {
            try {
                ...
                return formatDate(snapped, display_format || 'DD-MM-YYYY');
            } catch (err) {
                return `${timestamp}`;
            }
        };
