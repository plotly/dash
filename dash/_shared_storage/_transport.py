...
msgpack, or stdlib JSON as a fallback). The codec is **data-only, never pickle**
-- the socket is localhost-only, but deserializing pickle off a socket is a
remote-code-execution hazard, so the wire carries only data. The channel is
