...
class ComponentRegistry:  # pylint: disable=no-init,too-few-public-methods
    """Holds a registry of the namespaces used by components."""

    registry = set()
    __dist_cache = {}
    # noinspection PyTypeChecker
    _prop_changes_handlers = collections.defaultdict(
        lambda: collections.defaultdict(list)
    )
    _default_props_handlers = collections.defaultdict(dict)
    _prop_serializers = collections.defaultdict(dict)
    ...
    @classmethod
    def register_prop_change_handler(cls, typename, prop_name, handler):
        # type: (str, typing.Callable[[str, typing.Any, typing.Any, str], typing.Any]) -> None # noqa: E501
        cls._prop_changes_handlers[typename][prop_name].append(handler)

    @classmethod
    def register_prop_serializer(cls,
                                 typename,
                                 prop_name,
                                 serializer=None,
                                 deserializer=None):
        # type: (str, str, typing.Callable[[typing.Any], typing.Any], typing.Callable[[typing.Any], typing.Any]) -> None # noqa: E501
        cls._prop_serializers[typename][prop_name] = {
            'serializer': serializer,
            'deserializer': deserializer
        }

    @classmethod
    def register_default_prop(cls, typename, prop_name, default_prop):
        # type: (str, str, typing.Union[typing.Callable[[typing.Any], typing.Any], typing.Any]) -> None # noqa: E501
        cls._default_props_handlers[typename][prop_name] = default_prop

    @classmethod
    def on_prop_change(cls,
                       component,
                       prop_name,
                       new_value,
                       old_value,
                       prop_type):
        typename = getattr(component, '_typename', '')
        handlers = cls._prop_changes_handlers[typename][prop_name]

        for handler in handlers:
            handler(component, new_value, old_value, prop_type)

    @classmethod
    def deserialize_prop(cls, typename, prop_name, value):
        deserializer = cls._prop_serializers[
            typename][prop_name]['deserializer']
        if callable(deserializer):
            return deserializer(value)
        return value

    @classmethod
    def serialize_prop(cls, typename, prop_name, value):
        serializer = cls._prop_serializers[typename][prop_name]['serializer']
    ...
    def get_default_prop(cls, component, prop_name):
        default_prop = cls._default_props_handlers[
            component._typename
        ].get(prop_name)
        if callable(default_prop):
            return default_prop(component)
        return default_prop
...
def generate_imports(project_shortname, components):
    with open(os.path.join(project_shortname, '_imports_.py'), 'w') as f:
        f.write(textwrap.dedent(
            ...
            '''.format(
                '\n'.join('from .{} import {}'.format(
                    stringcase.snakecase(x), x) for x in components),
