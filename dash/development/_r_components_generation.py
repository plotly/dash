    ...
    if rpkg_data is not None and "r_examples" in rpkg_data:
        ...
        if the_ex and "code" in the_ex.keys():
            result += wrap(
                "examples",
                wrap(
                    "dontrun" if the_ex.get("dontrun") else "", the_ex["code"]
                ),
            )
...
def make_namespace_exports(components, prefix):
    ...
    export_string += "\n".join(
        "export({})".format(function) for function in fnlist
    )
...
def get_r_prop_types(type_object):
    ...
    return dict(
        ...
        ).format(get_r_type(type_object["value"])),
...
def get_wildcards_r(prop_keys):
    wildcards = ""
    wildcards += ", ".join(
        "'{}'".format(p) for p in prop_keys if p.endswith("-*")
    )
