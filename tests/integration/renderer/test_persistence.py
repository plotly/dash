    ...
    def layout():
        return html.Div(
            [
                html.Div(id="out"),
                simple_table(
                    persistence=app.persistence.value, **props_override
                ),
            ]
        )
...
def check_table_names(dash_duo, names, table_id="table"):
    ...
    headers = dash_duo.find_elements(
        "#{} .column-header-name".format(table_id)
    )
...
def test_rdps004_show_hide(dash_duo):
    ...
    @app.callback(
        Output("container", "children"), [Input("toggle-table", "n_clicks")]
    )
    def toggle_table(n):
        if (n or 0) % 2:
            return "nope"
...
def test_rdps006_move_on_page(dash_duo):
    ...
    @app.callback(
        Output("container", "children"), [Input("move-table", "n_clicks")]
    )
    def move_table(n):
...
def test_rdps008_unsaved_part_changed(dash_duo):
    ...
    @app.callback(
        Output("container", "children"), [Input("deletable", "n_clicks")]
    )
...
def test_rdps009_clear_prop_callback(dash_duo):
    ...
    @app.callback(
        Output("table", "columns"), [Input("reset-names", "n_clicks")]
    )
    def reset_names(n):
