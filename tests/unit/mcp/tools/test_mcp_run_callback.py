"""Callback dispatch execution via MCP tools (``run_callback``).

Exercises how the MCP tool pipeline runs a Dash callback through
``_process_mcp_message`` with various signatures: multi-output, State,
positional vs. dict-based ``inputs``, ``PreventUpdate``, and no-output
set_props-style callbacks.
"""
...
def test_mcpx001_multi_output():
    ...
    assert structured["response"]["dd2"]["options"] == [{"label": "b", "value": "b"}]
    assert structured["response"]["out"]["children"] == "selected: b"


def test_mcpx002_omitted_kwargs_default_to_none():
    app = Dash(__name__)
    app.layout = html.Div(
        [
...
def test_mcpx003_no_output_callback():
    app = Dash(__name__)
    app.layout = html.Div(
        [
            html.Button(id="btn"),
...
def test_mcpx004_prevent_update():
    app = Dash(__name__)
    app.layout = html.Div(
        [
...
def test_mcpx007_positional_inputs():
    app = Dash(__name__)
    app.layout = html.Div(
        [
    ...
    assert _call_tool_output(app, tool_name, {"val": "test"}, "pos-out") == "got:test"


def test_mcpx008_dict_inputs_with_state():
    app = Dash(__name__)
    app.layout = html.Div(
        [
