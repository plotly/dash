...
class TestPartialIdMatches:
    ...
    def test_partial_wildcard_match(self):
        ...
        pattern = Input({"type": MATCH}, "n_clicks", partial_pattern=True)
        component = Input({"type": "btn", "index": 1}, "n_clicks")
        assert pattern == component

    def test_partial_all_wildcard(self):
        """Partial pattern with ALL wildcard should match."""
        pattern = Input({"type": ALL}, "n_clicks", partial_pattern=True)
    ...
    def test_partial_no_common_keys(self):
        ...
        pattern = Input({"category": "nav"}, "n_clicks", partial_pattern=True)
        component = Input({"type": "btn", "index": 1}, "n_clicks")
        assert pattern != component

    def test_partial_exact_same_keys(self):
        """Partial with exact same keys should still work like normal."""
        pattern = Input({"type": MATCH, "index": 1}, "n_clicks", partial_pattern=True)
        component = Input({"type": "btn", "index": 1}, "n_clicks")
        assert pattern == component
    ...
    def test_partial_multiple_pattern_keys(self):
        ...
        pattern = Input(
            {"type": "btn", "page": "home"}, "n_clicks", partial_pattern=True
        )
        component = Input(
    ...
    def test_partial_output(self):
        """Output with partial_pattern=True should work."""
        pattern = Output({"type": "display"}, "children", partial_pattern=True)
        component = Output({"type": "display", "index": 1}, "children")
        assert pattern == component

    def test_partial_state(self):
        """State with partial_pattern=True should work."""
        pattern = State({"type": "store"}, "data", partial_pattern=True)
    ...
    def test_partial_to_dict_includes_flag(self):
        ...
        dep = Input({"type": MATCH}, "n_clicks", partial_pattern=True)
        d = dep.to_dict()
...
class TestPartialMatchSymmetry:
    ...
    def test_other_side_partial(self):
        """If the other side enables partial patterns, subset matching works."""
        pattern = Input({"type": "btn", "index": 1}, "n_clicks")
        component = Input({"type": "btn"}, "n_clicks", partial_pattern=True)
    ...
    def test_component_superset_of_pattern(self):
        ...
        pattern = Input({"type": MATCH}, "value", partial_pattern=True)
