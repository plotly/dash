"""
Basic WebSocket callback tests.

Tests:
- Per-callback websocket (websocket=True)
- Global websocket callbacks (websocket_callbacks=True)
- Mixed HTTP and WebSocket callbacks
"""

from dash import Dash, html, dcc, Input, Output, State, ctx


def test_ws001_per_callback_websocket(dash_duo):
    """Test single callback with websocket=True on FastAPI backend."""
...
def test_ws002_global_websocket_callbacks(dash_duo):
    """Test global websocket_callbacks=True enables WebSocket for all callbacks."""
    app = Dash(
        __name__,
        backend="fastapi",
...
def test_ws003_mixed_http_and_websocket(dash_duo):
    """Test mixing WebSocket and HTTP callbacks in the same app."""
    app = Dash(__name__, backend="fastapi")
...
def test_ws004_websocket_with_state(dash_duo):
    """Test WebSocket callback with State inputs."""
...
def test_ws005_websocket_context_available(dash_duo):
    """Test that WebSocket context is available in WebSocket callbacks."""
    app = Dash(__name__, backend="fastapi", websocket_callbacks=True)
...
def test_ws006_websocket_multiple_outputs(dash_duo):
    """Test WebSocket callback with multiple outputs."""
    app = Dash(__name__, backend="fastapi", websocket_callbacks=True)
...
def test_ws007_websocket_slider_callback(dash_duo):
    """Test WebSocket callback with dcc.Slider component."""
    app = Dash(__name__, backend="fastapi", websocket_callbacks=True)

    app.layout = html.Div(
        [
            dcc.Slider(id="slider", min=0, max=100, value=50, step=10),
            html.Div(id="output"),
        ]
    )
