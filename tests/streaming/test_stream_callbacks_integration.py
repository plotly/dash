...
def _downlink_connections(app):
    """Record the connection ids the downlink is polled with: the SharedWorker
    gives one browser one connection, however many tabs it has."""
    ...
    def _record():
        if request.path.endswith("_dash-update-component"):
            ...
            if "streamDownlink" in body:
                seen.add(body["streamDownlink"]["connectionId"])
